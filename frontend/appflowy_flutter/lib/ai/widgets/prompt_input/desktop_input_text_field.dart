...
class DesktopPromptInput extends StatefulWidget {
  const DesktopPromptInput({
    ...
    this.header,
    this.usePopoverDecorationStyle = false,
  });
  ...
  final Widget? header;
  final bool usePopoverDecorationStyle;
...
class _DesktopPromptInputState extends State<DesktopPromptInput> {
  ...
  void initState() {
    ...
    focusNode.addListener(
      () {
        if (!widget.usePopoverDecorationStyle) {
  ...
  BoxDecoration decoration(BuildContext context) {
    return BoxDecoration(
      ...
      border: Border.all(
        color: widget.usePopoverDecorationStyle
            ? Theme.of(context).colorScheme.outline
            : focusNode.hasFocus
                ? Theme.of(context).colorScheme.primary
                : Theme.of(context).colorScheme.outline,
        width:
            !widget.usePopoverDecorationStyle && focusNode.hasFocus ? 1.5 : 1.0,
      ),
      borderRadius: const BorderRadius.all(Radius.circular(12.0)),
      boxShadow: widget.usePopoverDecorationStyle
          ? const [
              BoxShadow(
                offset: Offset(0, 4),
                blurRadius: 20,
